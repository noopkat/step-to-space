...
const refreshTokens = () => {
  return new Promise((resolve, reject) => {
    tableService.retrieveEntity(process.env.TABLE_NAME, 'data', 'current', (error, result) => {
      if (error) reject(error.context.errors[0]);
      ...
      client.refreshAccessToken(access_token, refresh_token)
        .then(({access_token, refresh_token}) => {
          ...
          tableService.mergeEntity(process.env.TABLE_NAME, entity, (error) => {
            if (error) return reject(error.context.errors[0]);
...
const storeLifetimeDistance = (distance) => {
  return new Promise((resolve, reject) => {
    tableService.retrieveEntity(process.env.TABLE_NAME, 'data', 'current', (error, result) => {
      ...
      tableService.mergeEntity(process.env.TABLE_NAME, entity, (error) => {
        return error ? reject(error.context.errors[0]) : resolve();
      });
    });
  });
};
...
const updateLifetimeDistance = () => {
  refreshTokens()
    ...
    .catch(error => console.log('error saving distance to db:', error.context.errors));
};
...
const updateHeartrate = () => {
  refreshTokens()
   ...
   .catch(error => console.log('error saving heartrate to db:', error.context.errors));
}
